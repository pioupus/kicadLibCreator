...
PartCreationRule::PartCreationRule(QString name) { this->name = name; }
...
PartCreationRuleResult PartCreationRule::setKicadDeviceFieldsByRule(
    QMap<QString, QString> OctopartSource) {
  PartCreationRuleResult result;

  result.designator =
      replaceVariable(targetRule_designator.join(" "), OctopartSource);
  ...
  result.manufacturer =
      replaceVariable(targetRule_manufacturer.join(" "), OctopartSource);
  result.display_value =
      replaceVariable(targetRule_display_value.join(" "), OctopartSource);
  result.description =
      replaceVariable(targetRule_description.join(" "), OctopartSource);
  result.lib_name =
      replaceVariable(targetRule_lib_name.join(" "), OctopartSource);
  result.datasheet =
      replaceVariable(targetRule_datsheet.join(" "), OctopartSource);
...
void PartCreationRuleList::loadFromFile(QString filename) {
  ...
  for (auto rName : ruleNames) {
    ...
    rule.targetRule_designator.append(
        settings.value("targetRule_designator").toStringList());
    rule.targetRule_name.append(
        settings.value("targetRule_name").toStringList());
    rule.targetRule_footprint.append(
        settings.value("targetRule_footprint").toStringList());
    rule.targetRule_datsheet.append(
        settings.value("targetRule_datsheet").toStringList());
    ...
    rule.targetRule_manufacturer.append(
        settings.value("targetRule_manufacturer").toStringList());
    rule.targetRule_display_value.append(
        settings.value("targetRule_display_value").toStringList());
    rule.targetRule_description.append(
        settings.value("targetRule_description").toStringList());
    rule.targetRule_lib_name.append(
        settings.value("targetRule_lib_name").toStringList());

    rule.key_octo_footprint_up.append(
        settings.value("key_octo_footprint_up").toStringList());
    rule.key_octo_descrption_up.append(
        settings.value("key_octo_descrption_up").toStringList());
    rule.key_octo_specification_up.append(
        settings.value("key_octo_specification_up").toStringList());

    rule.key_octo_footprint_down.append(
        settings.value("key_octo_footprint_down").toStringList());
    rule.key_octo_descrption_down.append(
        settings.value("key_octo_descrption_down").toStringList());
    rule.key_octo_specification_down.append(
        settings.value("key_octo_specification_down").toStringList());

    rule.links_category.append(settings.value("links_category").toStringList());
    rule.links_source_device.append(
        settings.value("links_source_device").toStringList());
  ...
  if (!globalRule_exists) {
    ...
    rule.targetRule_datsheet.append(
        "%rule.name%/%octo.manufacturer.saveFilename%/"
        "%octo.mpn.saveFilename%.pdf");
...
void PartCreationRuleList::saveFile(QString filename) {
  ...
  for (auto rule : ruleList) {
    ...
    settings.setValue("targetRule_display_value",
                      rule.targetRule_display_value);
    ...
    settings.setValue("key_octo_specification_up",
                      rule.key_octo_specification_up);

    settings.setValue("key_octo_footprint_down", rule.key_octo_footprint_down);
    settings.setValue("key_octo_descrption_down",
                      rule.key_octo_descrption_down);
    settings.setValue("key_octo_specification_down",
                      rule.key_octo_specification_down);
...
void PartCreationRuleList::modified() {
  ...
  for (int i = 0; i < ruleList.count(); i++) {
    ...
    for (auto cat : cats) {
      auto sl = cat.split("~");
      assert(sl.count() == 2);
      linkedCategoryDirectory.insert(sl[0], i);
    }
  }
}

QList<PartCreationRule> PartCreationRuleList::findRuleByCategoryID(
    QList<OctopartCategorie> &categoryIDs) {
...
PartCreationRule
PartCreationRuleList::getRuleByNameForAppliaction(QString ruleName) {
